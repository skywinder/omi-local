...
def test_child_env_for_offline_mode() -> None:
    ...
    assert child["OMI_OFFLINE_ALLOWED_ENDPOINTS"] == f"{cfg.dev_bind_host}:{cfg.backend_port}"
    for key in (
        *config.CORE_PROVIDER_ENV,
        "TYPESENSE_HOST",
        "TYPESENSE_API_KEY",
        "OMI_LLM_GATEWAY_URL",
        "OMI_LLM_GATEWAY_SERVICE_TOKEN",
        "SCREEN_FRAME_EGRESS_ENABLED",
...
def test_offline_mode_omits_unused_screen_frame_configuration() -> None:
    cfg = config.HarnessConfig(
        repo_root=REPO_ROOT,
        instance="default",
        provider_mode="offline",
    ...
    child = config.child_env_for(cfg)
    assert "SCREEN_FRAME_SIGNING_SECRET" not in child
    assert "SCREEN_FRAME_EGRESS_ENABLED" not in child
