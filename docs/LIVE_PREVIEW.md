...
Live STT также настраивается через **Настройки → Live STT** в аудиотеке.
[Поддерживаемые провайдеры и применение настроек](PROVIDERS.md#live-stt).
После первого сохранения в интерфейсе он управляет активным профилем;
прежний `live-stt.json` остаётся копией исходных параметров.
...
накопленной записи. Голоса сопоставляются с предыдущей разметкой по пересечению
интервалов; видимые номера выдаются при появлении первой реальной реплики.
Шумовые кластеры до начала речи номера не занимают. Одновременно выполняется
только один расчёт; с ростом записи задержка
