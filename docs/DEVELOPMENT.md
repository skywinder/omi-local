...
Текущий WhisperX 3.8.6/CPU/float32/batch 1 работает на Python 3.12.14 с локально
изменёнными `whisperx/asr.py` и `whisperx/transcribe.py`. Эти изменения не входят
в репозиторий; установка пакета из PyPI не воспроизводит проверенное окружение.
Общий установщик ML-зависимостей и моделей пока отсутствует.
...
pyannote с вложенными моделями должны быть скачаны заранее. WhisperX также использует
данные NLTK `punkt_tab`. Проверка импорта библиотек не подтверждает наличие всех
этих ресурсов; `start.command --check` проверяет сервисы Mac, а не готовность ML.
