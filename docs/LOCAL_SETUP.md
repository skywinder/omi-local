...
Инструкция описывает проверенный исходный MVP. Перенос в этот отдельный
репозиторий проверяется статически и локальными тестами; полный запуск с чистого
компьютера ещё не подтверждён.
...
- iPhone 17 Pro с Developer Mode, data-capable USB и доступом к той же LAN, что и Mac.
...
2. Возобновите запись кнопкой микрофона на карточке CV1. Нижний `+` включает
   микрофон телефона и не подходит для проверки CV1.
3. Запишите 10–30 секунд тестовой речи.
4. Закройте приложение через переключатель приложений iPhone. На проверенной
   версии Mute, выключение CV1 и Disconnect останавливают аудиофреймы, но могут
   оставить WebSocket открытым и не завершить файл.
...
Подтверждён результат: 745 аудиофреймов, 238 400 samples, 14,9 секунды,
PCM16 mono 16 kHz. Это одна физическая сессия; тесты повторяемости остаются открытыми.
