# Omi Local: scope of this standalone snapshot

This repository contains the local iOS/CV1 audio MVP. Read `README.md` and
`docs/LOCAL_SETUP.md` first. If a local, ignored `Work/STATE.md` exists, it is
the single source for the active gate and private working context.

The upstream guide below is preserved. Its cloud deployments, missing monorepo
components, upstream remotes, and `make setup`/PR automation do not apply here.
...
- Before any iOS build record `build`, `reuse`, or `blocked` from actual inputs.
  ...
  backend failures alone do not justify rebuilding or cleaning caches.
- Before installation record commit, configuration, exact artifact, executable
  SHA-256, signature verification, and entitlements; install only that artifact.
- Classify results as `passed`, `failed`, or `indeterminate`. Validate observer
  capability before host-sensitive checks; sandbox failures alone are inconclusive.
- Keep real recordings, logs, identifiers, signing material, and working notes
  out of Git. Never print or persist private identifiers; use them transiently
  only when required by a system API. Keep public docs short and evidence-based.
- Do not mark physical no-egress, reliability, or clean-machine setup as passed
  without fresh evidence. Preserve existing working history and agent guidance.
