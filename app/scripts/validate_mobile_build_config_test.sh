...
offline_output="$(
  cd "$fixture_dir"
  "$VALIDATOR" --flavor dev --profile local_dev --runtime-mode offline
)"
[[ "$offline_output" == *'runtime=offline'* ]]
...
if (
  cd "$fixture_dir"
  "$VALIDATOR" --flavor dev --profile local_dev --runtime-mode offline
) 2>"$fixture_dir/public-offline.err"; then
  echo 'FAIL: accepted a public API host for offline runtime' >&2
  exit 1
fi
grep -F 'loopback/private host' "$fixture_dir/public-offline.err" >/dev/null
...
if (
  cd "$fixture_dir"
  "$VALIDATOR" --flavor prod --profile mobile_beta --runtime-mode offline
) 2>"$fixture_dir/offline-profile.err"; then
  echo 'FAIL: accepted offline runtime with the production flavor' >&2
  exit 1
fi
grep -F 'offline runtime requires flavor=dev' "$fixture_dir/offline-profile.err" >/dev/null
