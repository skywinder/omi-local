...
class GeminiStreamingSttSocket implements IPureSocket {
  ...
  Future<bool> connect() async {
    ...
    try {
      OfflineNetworkPolicy.current.requireAllowed(Uri.parse(_wsUrl));
    } on OfflineNetworkBlocked catch (error) {
      CustomSttLogService.instance.error('GeminiStreaming', error.toString());
      return false;
    }
...
class PureStreamingSttSocket implements IPureSocket {
  ...
  Future<bool> connect() async {
    ...
    try {
      OfflineNetworkPolicy.current.requireAllowed(Uri.parse(config.url));
    } on OfflineNetworkBlocked catch (error) {
      CustomSttLogService.instance.error(config.serviceId, error.toString());
      return false;
    }
