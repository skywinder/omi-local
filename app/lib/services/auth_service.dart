...
class AuthService {
  ...
  Future<UserCredential?> signInWithGoogleMobile() async {
    Env.requireExternalAuthAllowed();
  ...
  Future<UserCredential> signInWithLocalEmulator() async {
    Env.validateOfflineRuntimePolicy(
      configuredProfile: Env.profile,
      configuredApiBaseUrl: Env.apiBaseUrl,
      authEmulatorHost: Env.firebaseAuthEmulatorHost,
      testUser: Env.localTestUser,
    );
    Env.validateFirebaseProject(projectId: Firebase.app().options.projectId);
    const alias = Env.localTestUser;
    final credentials = localEmulatorCredentialsFor(alias);
    final result = await FirebaseAuth.instance.signInWithEmailAndPassword(
      email: credentials.email,
      password: credentials.password,
    );
    await _updateUserPreferences(result, 'local_emulator');
    return result;
  }
