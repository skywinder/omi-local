...
class AuthenticationProvider extends BaseProvider {
  ...
  Future<void> onLocalEmulatorSignIn(Function() onSignIn) async {
    ...
    setLoadingState(true);
    try {
      final credential = await AuthService.instance.signInWithLocalEmulator();
      if (_hasFirebaseUser) {
        await _signIn(onSignIn, credential: credential, authProvider: 'local_emulator');
      } else {
        throw StateError('Firebase Auth emulator did not return an authenticated user.');
      }
    } catch (e) {
      Logger.debug('Local emulator sign in error: ${e.runtimeType}');
      AppSnackbar.showSnackbarError(
        globalNavigatorKey.currentContext?.l10n.authenticationFailed ?? 'Authentication failed. Please try again.',
      );
    } finally {
