...
void main() {
  ...
  group('mobile environment profiles', () {
    test('runtime mode parser is fail-closed', () {
      expect(OmiRuntimeMode.parse(''), OmiRuntimeMode.standard);
      expect(OmiRuntimeMode.parse('standard'), OmiRuntimeMode.standard);
      expect(OmiRuntimeMode.parse('offline'), OmiRuntimeMode.offline);
      expect(() => OmiRuntimeMode.parse('production'), throwsStateError);
    });

    test('offline runtime blocks external OAuth', () {
      expect(() => Env.requireExternalAuthAllowedFor(OmiRuntimeMode.offline), throwsStateError);
      expect(() => Env.requireExternalAuthAllowedFor(OmiRuntimeMode.standard), returnsNormally);
    });

    test('offline runtime accepts only the paired local profile, host, and alice fixture', () {
      expect(
        () => Env.validateOfflineRuntimePolicy(
          configuredProfile: AppEnvironmentProfile.localDev,
          configuredApiBaseUrl: 'http://192.168.1.20:8000/',
          authEmulatorHost: '192.168.1.20',
          testUser: 'alice',
        ),
        returnsNormally,
      );
    ...
    test('offline runtime rejects profile, public host, host mismatch, and unknown user', () {
      final invalidCases = <({AppEnvironmentProfile profile, String api, String authHost, String user})>[
        (
          profile: AppEnvironmentProfile.mobileBeta,
          api: 'http://192.168.1.20:8000/',
          authHost: '192.168.1.20',
          user: 'alice',
        ),
        (
          profile: AppEnvironmentProfile.localDev,
          api: 'https://api.omiapi.com/',
          authHost: '192.168.1.20',
          user: 'alice',
        ),
        (
          profile: AppEnvironmentProfile.localDev,
          api: 'http://192.168.1.20:8000/',
          authHost: '192.168.1.21',
          user: 'alice',
        ),
        (profile: AppEnvironmentProfile.localDev, api: 'http://127.0.0.1:8000/', authHost: '127.0.0.1', user: 'bob'),
      ];
      for (final value in invalidCases) {
        expect(
          () => Env.validateOfflineRuntimePolicy(
            configuredProfile: value.profile,
            configuredApiBaseUrl: value.api,
            authEmulatorHost: value.authHost,
            testUser: value.user,
          ),
          throwsStateError,
        );
