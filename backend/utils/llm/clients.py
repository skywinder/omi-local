...
def get_llm(
    ...
    """
    environment_stage = os.environ.get('OMI_ENV_STAGE', '').strip().lower()
    provider_mode = os.environ.get('PROVIDER_MODE', '').strip().lower()
    if environment_stage == 'offline' or provider_mode == 'offline':
        raise RuntimeError(
            "Direct LLM access is disabled in offline runtime; use an explicit local fake at the call site."
        )
