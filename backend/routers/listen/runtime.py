...
class ListenSessionRuntime:
    ...
    def __init__(self, request: ListenRequest):
        ...
        self.original_codec = request.codec
        self.offline_runtime = is_offline_runtime()
    ...
    def start_live_transcription(self) -> None:
        ...
        if self.use_custom_stt or getattr(self, 'offline_runtime', False):
            return
    ...
    def capture_decode_error(self, *, encoded_bytes: int) -> None:
        if self.capture_sink is not None:
            self.capture_sink.record_decode_error(encoded_bytes=encoded_bytes)
    ...
    async def _bootstrap(self) -> bool:
        ...
        if getattr(self, 'offline_runtime', False):
            self.stt_service = 'offline_capture'
    ...
    def _ready_event(self) -> MessageServiceStatusEvent:
        ...
        """
        if getattr(self, 'offline_runtime', False):
            return MessageServiceStatusEvent(status='ready', provider='offline_capture')
        if self.use_custom_stt:
    ...
    async def run(self) -> None:
        ...
        try:
            try:
                ...
                logger.error('Offline audio capture initialization failed type=%s', type(error).__name__)
    ...
    async def _teardown(self) -> None:
        try:
            ...
            try:
                if self.capture_sink is not None:
                    await run_blocking(storage_executor, self.capture_sink.finalize)
            except Exception as error:
                logger.error('Offline audio capture finalization failed type=%s', type(error).__name__)
            finally:
