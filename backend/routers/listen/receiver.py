...
class ListenReceiver:
    ...
    def _record_decode_failure(
        ...
        """
        capture_error = getattr(self.host, 'capture_decode_error', None)
        if callable(capture_error):
            capture_error(encoded_bytes=payload_len)
    ...
    async def initialize_stt(self) -> bool:
        request = self.host.request
        if self.host.use_custom_stt or getattr(self.host, 'offline_runtime', False):
            return True
        try:
            if self.host.is_multi_channel:
    ...
    async def _handle_multi_channel_audio(self, data: bytes, now: float | None = None) -> int:
        ...
        if not self.host.use_custom_stt and not getattr(self.host, 'offline_runtime', False):
        ...
        try:
            while self.host.state.active:
                ...
                if data is not None:
                    ...
                    if not decoded:
                        continue
                    capture_decoded = getattr(self.host, 'capture_decoded_audio', None)
                    if callable(capture_decoded):
                        capture_decoded(encoded_bytes=len(data), pcm=decoded)
                    self._mark_first_audio(now)
