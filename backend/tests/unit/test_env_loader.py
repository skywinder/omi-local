...
def test_is_offline_runtime_uses_resolved_explicit_or_provider_stage() -> None:
    assert is_offline_runtime({"OMI_ENV_STAGE": "offline"}) is True
    assert is_offline_runtime({"PROVIDER_MODE": "offline"}) is True
    assert is_offline_runtime({"OMI_ENV_STAGE": "local", "PROVIDER_MODE": "offline"}) is False
