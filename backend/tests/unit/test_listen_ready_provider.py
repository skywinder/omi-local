...
@pytest.mark.anyio
async def test_offline_bootstrap_never_selects_an_external_stt_provider(monkeypatch):
    """The real offline listen route is capture-only, even with provider placeholders."""
    import routers.listen.runtime as runtime_module
    from utils.listen_session_bootstrap import ListenConnectBase

    runtime = object.__new__(ListenSessionRuntime)
    runtime.request = ListenRequest(websocket=SimpleNamespace(), uid='offline-user', source='omi', codec='opus_fs320')
    runtime.use_custom_stt = False
    runtime.offline_runtime = True
    runtime.session_id = 'offline-session'
    runtime.language = 'en'
    ...
    monkeypatch.setattr(
        runtime_module,
        'get_stt_service_for_language',
        lambda *_a, **_k: (_ for _ in ()).throw(AssertionError('offline bootstrap selected external STT')),
    )
    monkeypatch.setattr(runtime_module, 'finalize_listen_connect_context', _stop_after_selection)
    ...
    assert runtime.stt_service == 'offline_capture'
    assert runtime.stt_service_selected == 'offline_capture'
    assert runtime._ready_event().to_json()['provider'] == 'offline_capture'


@pytest.mark.anyio
async def test_offline_receiver_initialization_creates_no_stt_socket():
    host = SimpleNamespace(
        request=ListenRequest(websocket=SimpleNamespace(), uid='offline-user'),
        use_custom_stt=False,
        offline_runtime=True,
    )
    receiver = ListenReceiver(host, [], {})
    receiver._create_stt_socket = AsyncMock(side_effect=AssertionError('external STT socket attempted'))

    assert await receiver.initialize_stt() is True
    receiver._create_stt_socket.assert_not_awaited()
