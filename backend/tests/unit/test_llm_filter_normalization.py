...
def test_offline_initial_message_is_local_and_does_not_resolve_an_llm(monkeypatch):
    monkeypatch.setenv('OMI_ENV_STAGE', 'offline')
    monkeypatch.setattr(chat, 'get_prompt_memories', lambda _uid: pytest.fail('offline greeting read memories'))
    monkeypatch.setattr(chat, 'get_llm', lambda _feature: pytest.fail('offline greeting resolved an LLM'))

    assert chat.initial_chat_message('alice') == 'Omi Local is ready.'


def test_get_llm_fails_closed_when_either_offline_invariant_is_active(monkeypatch):
    monkeypatch.setenv('PROVIDER_MODE', 'offline')

    with pytest.raises(RuntimeError, match='Direct LLM access is disabled in offline runtime'):
        clients.get_llm('chat_responses')
