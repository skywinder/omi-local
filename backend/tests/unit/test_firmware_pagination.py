...
@pytest.mark.asyncio
async def test_offline_mode_returns_no_releases_without_cache_or_http() -> None:
    with patch.dict(os.environ, {"OMI_ENV_STAGE": "offline"}, clear=False), patch(
        'utils.github_releases.get_generic_cache'
    ) as mock_cache, patch('utils.github_releases.get_web_fetch_client') as mock_client:
        result = await get_omi_github_releases("offline_key", tag_filter=FIRMWARE_TAG_PATTERN)
